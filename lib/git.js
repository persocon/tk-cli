...
class Git {
  ...
  pushToMaster() {
    ...
    // can't handle error at all because git seems to put everything into stderr anyways
    console.log(chalk.white.bgBlue(' Pushing package.json to Master '));
    // if(p.stderr.toString() && p.stderr.toString().split('\n')[0] !== "Everything up-to-date" ) {
  ...
  pushTag() {
    ...
    // again, can't check for errors because aparently everything goes to stderr
    console.log(chalk.white.bgBlue(` Pushing tag ${this.version.bumpVersion} `));
